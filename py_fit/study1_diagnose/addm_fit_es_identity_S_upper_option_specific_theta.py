...
def validate_input(data: pd.DataFrame) -> dict:
    """
    ES identity-coordinate option-specific-theta aDDM.
    ...
    Interpretation:
        smaller theta_S -> S loses more influence when S is unattended
        smaller theta_E -> E loses more influence when E is unattended
    """
    ...
    if missing:
        raise ValueError(
            "Missing required columns for option-specific theta model: "
            f"{missing}"
        )
    ...
    # Verify the two option-specific unattended components really reconstruct
    # the common unattended term if that column is present.
    reconstruction_diff = None

    if "InattentionW_SE" in data.columns:
        common_inattention = pd.to_numeric(
        ...
        reconstruction_diff = float(
            np.nanmax(
                np.abs(
                    split_sum.to_numpy()
                    - common_inattention.to_numpy()
                )
            )
        )

        if reconstruction_diff > 0.001 + 1e-9:
            raise ValueError(
                "Unattended_S + Unattended_E does not reproduce "
                "InattentionW_SE within rounding tolerance. "
                f"Max difference={reconstruction_diff:.6f}"
            )
    ...
    audit = {
        ...
        "drift_formula": (
            "v ~ 1 + AttentionW_SE + Unattended_S + Unattended_E"
        ),
        "parameter_interpretation": {
            "v_Intercept": (
                "b0: residual drift bias; positive is toward S, "
                "negative is toward E"
            ),
            "v_AttentionW_SE": (
                "bA: common attended-value sensitivity"
            ),
            "v_Unattended_S": (
                "bIS: sensitivity to S value while S is unattended"
            ),
            "v_Unattended_E": (
                "bIE: sensitivity to E value while E is unattended"
            ),
            "theta_S": "bIS / bA",
            "theta_E": "bIE / bA",
            "delta_theta_S_minus_E": "theta_S - theta_E",
        },
        "max_abs_split_inattention_reconstruction_difference": (
            reconstruction_diff
        ),
    }
...
def compute_design_diagnostics(
    ...
    """
    Check the three drift predictors before MCMC.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Fit ES S-upper/E-lower option-specific-theta aDDM "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Validate the input and design matrix, then stop before MCMC."
        ),
    )
    ...
    print(
        "\nAll S-upper/E-lower option-specific-theta chains finished.",
        flush=True,
    )
