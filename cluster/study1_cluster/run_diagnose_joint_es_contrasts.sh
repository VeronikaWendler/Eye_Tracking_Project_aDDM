#!/bin/bash
#SBATCH --job-name=diag_es_contr
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem=32G
#SBATCH --output=diag_es_contr_%j.out
#SBATCH --error=diag_es_contr_%j.err
...
MODEL_DIR_HOST="${MODEL_DIR_HOST:-${OUT_DIR_HOST}/models_ES_joint_es_contrasts_final}"
DIAG_DIR_HOST="${DIAG_DIR_HOST:-${OUT_DIR_HOST}/figures/aDDM_ES_joint_es_contrasts/diagnostics}"
CHAINS="${CHAINS:-3}"
...
apptainer exec --cleanenv \
  ...
  "${IMAGE}" \
  python /workspace/addm_diagnose_joint_es_contrasts.py \
    --model-dir /target_models \
    --out-dir /diagnostics \
    --chains "${CHAINS}"

echo "Finished joint E/S contrast diagnostics."
