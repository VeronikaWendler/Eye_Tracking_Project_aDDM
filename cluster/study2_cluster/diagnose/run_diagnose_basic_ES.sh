#!/bin/bash
#SBATCH --job-name=s2_diag_basic_ES
#SBATCH --time=06:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem=32G
#SBATCH --output=/rds/projects/z/zhanglp-vwendler-core/Study2_aDDM/derivatives/logs/diagnostics/basic_ES_%j.out
#SBATCH --error=/rds/projects/z/zhanglp-vwendler-core/Study2_aDDM/derivatives/logs/diagnostics/basic_ES_%j.err
...
PHASE="ES"
MODEL_DIR_HOST="${OUT_DIR_HOST}/models/basic_ES"
FIG_DIR_HOST="${OUT_DIR_HOST}/figures/basic_ES"
CHAINS="${CHAINS:-3}"
...
for i in $(seq 0 $((CHAINS - 1))); do
  [[ -f "${MODEL_DIR_HOST}/basic_aDDM_ES_${i}.hddm" ]] || {
    echo "ERROR: missing chain ${i}: ${MODEL_DIR_HOST}/basic_aDDM_ES_${i}.hddm" >&2
    exit 1
  }
done
