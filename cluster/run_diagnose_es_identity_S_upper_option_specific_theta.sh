#!/bin/bash
#SBATCH --job-name=diag_es_id_theta
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem=32G
#SBATCH --output=diag_es_id_theta_%j.out
#SBATCH --error=diag_es_id_theta_%j.err
...
DIAG_DIR_HOST="${DIAG_DIR_HOST:-${OUT_ROOT}/figures/aDDM_ES_identity_S_upper_option_specific_theta/diagnostics}"
...
apptainer exec --cleanenv \
  ...
  "${IMAGE}" \
  python /workspace/addm_diagnose_es_identity_S_upper_option_specific_theta.py \
    --model-dir /target_models \
    --out-dir /diagnostics \
    --chains "${CHAINS}"

echo "Finished S-upper/E-lower option-specific-theta diagnostics."
